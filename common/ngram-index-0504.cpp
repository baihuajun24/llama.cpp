...
void NGramIndex::add_token(const llama_token* context, int context_size, llama_token new_token) {
    // Validate context size
    if (context_size < 0) {
        LOG_ERR("Invalid context size: %d", context_size);
        return;
    }
    
    // Get the source ID and position for this token
    int source_id;
    size_t position;
    
    // Special case: empty context (first token)
    if (context_size == 0) {
        // Create a new prompt with just this token
        std::vector<llama_token> initial_tokens = {new_token};
        source_id = source_manager.add_virtual_prompt(initial_tokens);
        position = 0;
    }
    // Check if we have any existing virtual prompts
    else if (source_manager.get_virtual_prompt_count() == 0) {
        // No prompts yet, create a new one with this token
        std::vector<llama_token> initial_tokens = {new_token};
        source_id = source_manager.add_virtual_prompt(initial_tokens);
        position = 0;
    } 
    else {
        // Always use first prompt ID (0) for simplicity in this implementation
        source_id = 0;
        
        // Append the token to the existing prompt
        bool success = source_manager.append_token_to_prompt(source_id, new_token);
        if (!success) {
            // If append fails (shouldn't happen), create a new prompt
            LOG_INF("Creating new virtual prompt for token");
            std::vector<llama_token> new_prompt = {new_token};
            source_id = source_manager.add_virtual_prompt(new_prompt);
            position = 0;
        } else {
            // Position will be at the end of the context
            position = context_size;
        }
    }
    
    // Special case: if context_size is 0, we can only create 1-grams
    if (context_size == 0) {
        if (ngram_min <= 1 && ngram_max >= 1) {
            // Create a 1-gram with just the new token
            ngram_index_key key(&new_token, 1);
    ...
    // Add ngrams of different sizes ending with the new token
    for (int n = ngram_min; n <= ngram_max && n <= context_size + 1; ++n) {
        // Calculate how many tokens we need from the context
        int context_tokens_needed = n - 1;
        
        // Skip if we don't have enough context
        if (context_tokens_needed > context_size) {
            continue;
        }
        
        // Calculate the start position for this n-gram in the context
        int start_idx = context_size - context_tokens_needed;
        
        // Create a buffer for this n-gram (context tokens + new token)
        std::vector<llama_token> ngram_tokens;
        ngram_tokens.reserve(n);
        
        // Add context tokens
        for (int i = 0; i < context_tokens_needed; ++i) {
            ngram_tokens.push_back(context[start_idx + i]);
        }
        
        // Add the new token
        ngram_tokens.push_back(new_token);
        
        // Create an n-gram key
        ngram_index_key key(ngram_tokens.data(), n);
        
        // Add the location to the index
        // The position is the position of the first token in the n-gram in the source
        index[key].add(ngram_location(
            ngram_location::StorageType::VIRTUAL_PROMPT,
            source_id,
            position - (n - 1)  // Position of the first token in the n-gram
        ));
    }
}
...
int NGramIndex::draft_multiple(const llama_token* tokens, int n_tokens, int n_draft, 
    ...
    for (int n = std::min(n_tokens, ngram_max); n >= ngram_min; --n) {
        ...
        if (it != index.end() && !it->second.empty()) {
            ...
            if (source_id < 0 || (size_t)source_id >= source_manager.get_virtual_prompt_count()) {
                continue;
            }
            ...
            for (int i = 0; i < n_draft; ++i) {
                // Get the next token
                ngram_location next_loc(loc.type, source_id, position + n + i);
                llama_token next_token = source_manager.get_token_at_location(next_loc);
